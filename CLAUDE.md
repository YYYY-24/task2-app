...
### Step 3: Logic & Data
- データ項目: プロジェクト（名称、着工予定、完成予定、次回打ち合わせ日〈日付＋時刻〉）、業務項目（プロジェクトに紐づく、期限〈日付＋時刻〉、優先度、担当者、メモ）
  - 着工予定・完成予定・次回打ち合わせ日はいずれもプロジェクト単位の情報。登録時に入力できるほか、プロジェクト詳細画面内でいつでも修正できる
  - プロジェクト名称もプロジェクト詳細画面（見出し横の鉛筆アイコン）から後から変更できる
- 期限なし業務項目の優先度: 業務項目登録時に「日付を指定／早めに対応／急がない」を選べる（`tasks.priority`、`due_at`がある場合は使用しない）。「早めに対応」「急がない」はスケジュール画面で期限ありの項目とは別の見出し・配色でグループ表示し、期間フィルターに関係なく常に表示する
- サンプルデータ:
...
- 完了管理: 業務項目にはチェックボックスで完了/未完了を切り替えられる（`tasks.completed_at`、完了時刻を保持しnullなら未完了）。完了にするとスケジュール画面・プロジェクト一覧の件数/直近期限からは自動的に非表示になる。スケジュール画面・プロジェクト内の業務リスト画面どちらにも「完了済みを表示」トグルがあり、オンにすると期間フィルターに関係なく見返せる（表示のみで再度チェックを外せば未完了に戻せる）
- 削除: 誤って作成したプロジェクト・業務項目は削除できる。プロジェクト削除時は紐づく業務項目も連鎖削除される（`on delete cascade`）。削除は確認ダイアログを挟む
- 編集: スケジュール画面・プロジェクト内の業務リスト画面どちらでも、各業務項目の鉛筆アイコンから名称・期限（日付を指定／早めに対応／急がない）・担当者・メモをその場で編集できる（共通の`TaskRow`コンポーネント）
- メモ: 業務項目にはタイトルとは別に自由記述のメモ（`tasks.notes`）を登録できる。一覧ではタイトルより小さく薄い色で、タイトルの下に表示する
